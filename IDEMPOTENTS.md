...
## 2. idem-core Kütüphanesine Özel Matematiksel Formülasyon

`idem-core` kütüphanesi kapsamında kısıtlar, durum uzayının belirli bir afin veya diferansiyellenebilir manifold alt kümesine $\mathcal{M}$ hapsedilmesi şeklinde modellenir.
...
Aşağıdaki tabloda ve ayrıntılı alt bölümlerde `idem-core` kütüphanesinde kullanılan tüm idempotent manifoldlar ve bunların `src/` dizinindeki birebir karşılıkları verilmiştir:

| No | Manifold Adı | Sembol | `src/` Karşılığı | Karmaşıklık | Kısıt Tipi |
| :-: | :--- | :---: | :--- | :---: | :--- |
| **1** | **Permütasyon-generate_idempotent_map** | $\mathcal{M}_{genera}$ | `idempotent_core.binding:generate_idempotent_map` | $O(N)$ | Kapalı Konveks Alt-Uzay |
| **2** | **Permütasyon-compact_inplace** | $\mathcal{M}_{compac}$ | `idempotent_core.binding:compact_inplace` | $O(N)$ | Kapalı Konveks Alt-Uzay |

### 3.1. Permütasyon-generate_idempotent_map ($\mathcal{M}_{genera}$)
- **`src/` Karşılığı:** Sınıf/Fonksiyon: [`generate_idempotent_map`](file:///packages/idem-core/src/idempotent_core/binding.py), Modül: `idempotent_core.binding`
- **Fiziksel / Algoritmik Anlam:** Constructs an idempotent projection map f(x) for Top-K capacity.
...
### 3.2. Permütasyon-compact_inplace ($\mathcal{M}_{compac}$)
- **`src/` Karşılığı:** Sınıf/Fonksiyon: [`compact_inplace`](file:///packages/idem-core/src/idempotent_core/binding.py), Modül: `idempotent_core.binding`
- **Fiziksel / Algoritmik Anlam:** In-Place Zero-Copy Compaction.
